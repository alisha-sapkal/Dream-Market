...
function UpcomingProjects() {
  return (
    ...
    >
      <h1 className="text-3xl">HAVE A PROPERTY</h1>
      <h1 className="text-2xl">POST IT ON OUR NETWORK</h1>
      <div className="flex flex-row justify-between md:justify-evenly gap-4 font-semibold text-black p-6">
        <button className="bg-gray-100 rounded-full p-2 px-4 w-[100px]">
          SELL
        </button>
        <button className="bg-gray-100 rounded-full p-2 px-4 w-[100px]">
          RENT
      ...
      <motion.div
        className="max-w-6xl mx-auto overflow-x-auto scrollbar-hide px-2 sm:px-1 h-[320px] sm:h-[410px]"
        style={{
        ...
        <div className="flex gap-4 sm:gap-6 w-max h-full items-start py-2 sm:py-4">
          <AnimatePresence>
            {properties.map((offer, idx) => (
              <motion.div
                key={offer.type}
                className="group rounded-2xl bg-white/30 text-start backdrop-blur-md shadow-lg flex flex-col items-stretch overflow-hidden sm:px-2 p-2 min-w-[280px] w-[280px] sm:min-w-[320px] sm:w-[320px] h-[300px] sm:h-[350px]"
