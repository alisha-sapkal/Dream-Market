		...
		<section className="w-full bg-white rounded-2xl shadow-sm py-8 px-2 sm:px-6 md:px-12 my-6">
			...
			<div className="flex flex-col md:flex-row justify-center gap-8 md:gap-16 max-w-5xl mx-auto">
				{topLinks.map((col, idx) => (
					<div
						key={col.title}
						className="flex-1 min-w-[180px] flex flex-col items-center text-center md:items-start"
					>
						<h3 className="font-semibold text-base sm:text-lg mb-3 sm:text-start md:text-left">
							{col.title}
						</h3>
						<ul className="space-y-2 w-full text-start">
							{col.links.map((link, i) => (
								<li
									key={i}
									className="text-gray-700 text-sm sm:text-base text-start"
								>
									{link}
								</li>
							))}
